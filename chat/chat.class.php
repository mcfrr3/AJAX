...
/**
 * Class chat
 * contains server-side chat functionality
 */
class Chat
{
	// database handler
	...
	/**
	 * The isDatabaseCleared method checks to see if the database has
	 * been cleared since last call to the server
	 * - the $id parameter contains the id of the last message received
	 * by the client
	 */
	private function isDatabaseCleared($id){
		if($id > 0){
			// by checking hte number of rows with ids smaller than the
